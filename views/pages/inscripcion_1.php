...
<head>
    ...
    <link rel="stylesheet" href="<?= BASE_URL ?>assets/css/inscripcion_2.css?v=<?= time() ?>" />
</head>
<body>
...
<div class="container">
    ...
    <form action="<?= BASE_URL ?>?controller=home&action=inscripcion_1" method="POST">
        ...
        <?php endif; ?>
        <input type="hidden" name="semestre" value="<?= htmlspecialchars($_GET['semestre'] ?? '1') ?>" />
        ...
        <fieldset>
            ...
            <div class="form-group">
                <h3 for="referentes">Interdisciplinariedad del proyecto:</h3>
                <textarea id="referentes" name="referentes" placeholder="Mencione el nombren y cómo se aplicó en el proyecto los conocimientos adquiridos en las diferentes asignaturas vista dentro del mismo Programa " required></textarea> 
            </div>

            <div class="form-group">
                <h3 for="metodologia">Desarrollo de proyectos:</h3>
                <textarea id="metodologia" name="metodologia" placeholder="Mencione Tipo de investigación, Población objeto, Procedimiento para el abordaje de los objetivos, Instrumento de recolección de datos" required></textarea>
            </div>

            <div class="form-group">
                <h3 for="resultados">Resultados Esperados:</h3>
                <textarea id="resultados" name="resultados" placeholder="Se evaluarán los resultados esperados acorde a los objetivos específicos y al desarrollo del proyecto"required></textarea>
            </div>
